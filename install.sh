...
#              duplicated in this script for the agent role). Requires
#              TOKEN=<token>, generated by the mesh admin UI's "Node Agent"
#              panel on a node. PORT=<port> optionally overrides the default
#              (9200). There's no MESH=<url> to set: the agent is pulled by
#              the mesh on its existing poll cycle, it never needs to know
#              the mesh's own address (see .local/specs/node-agent.md
#              section 3 - pull-only transport, by design, not an omission).
#              Example: curl ... | ROLE=agent TOKEN=xxxxx sh
#
...
if [ "$ROLE" = "agent" ]; then
  if [ -z "$TOKEN" ]; then
    echo ""
    echo "Error: ROLE=agent requires TOKEN=<token>."
  ...
  if [ "$(id -u)" = "0" ]; then
    "$BIN_PATH" agent service install --port="$AGENT_PORT" --token="$TOKEN"
  elif command -v sudo >/dev/null 2>&1; then
    sudo "$BIN_PATH" agent service install --port="$AGENT_PORT" --token="$TOKEN"
  else
