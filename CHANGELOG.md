...
### Fixed
- **System Audit Trail's action-type dropdown blurred the table rows behind it when opened.** Its table wrapper was the only one in the app still using a translucent `bg-card/30 backdrop-blur-sm` background instead of the solid `bg-card` every other page's data table uses, so an open dropdown panel bled a frosted-glass effect onto the rows underneath it. Now matches the rest of the app.
