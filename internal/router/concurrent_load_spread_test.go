...
// This is a deterministic router-level test using synthetic NodeState: it
// does not require real vLLM/TGI hardware. It is NOT proof of real vLLM/TGI
// fleet behavior under real GPU load, which remains a separate, still-open
// validation item tracked in the internal queue.

// TestConcurrentLoadSpreadsAcrossNodes: 50 concurrent requests against N
// equally-capable nodes must not collapse onto one node. This exercises the
// pre-existing ActiveConns/TTFT-weighted spread scoring, independent of the
// predicted-queue-delay work.
func TestConcurrentLoadSpreadsAcrossNodes(t *testing.T) {
	const numNodes = 5
...
//
//   - This scenario's own text ties its "50-concurrent load" to a hardware
//     bench that is itself blocked on hardware - a real live load
//     test measuring p95 TTFT direct-to-backend vs. via-marbor-across-N-nodes,
//     not an abstract scoring-function comparison. Neither this scenario nor the
...
//
// Validating the actual claim requires the real hardware bench
// (direct-to-backend vs via-marbor p95 TTFT, with and without this scoring
// change) - it remains blocked on that hardware. See the internal queue
