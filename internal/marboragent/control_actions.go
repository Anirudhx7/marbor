...
// runtime.start/runtime.stop/runtime.restart. Same map-keyed-by-driver-name style
...
// the ControlDriver fresh per-request from exactly what marbor tells it.
...
// "Runtime control unavailable" error, never a guess.
//
// No self-healing: nothing in this file calls a ControlDriver
...
func (s *Server) handleRuntimeAction(w http.ResponseWriter, r *http.Request, action string) {
	...
	// This is the exact "Runtime control unavailable" error, never a
	// guessed driver.
	if req.Driver == "" {
...
// mutates the node - but still needs marbor to inject driver/identifier on
