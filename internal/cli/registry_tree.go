...
func buildRoot() *Command {
	rootBuilt = true
	authFlags := "requires credentials: run \"ollama-mesh login\" once (recommended), or pass --username+--password (or MESH_USERNAME+MESH_PASSWORD)."

	r := &Command{
		Name:  "ollama-mesh",
		Short: "CLI client for the ollama-mesh Admin API",
...
// so "ollama-mesh completion bash" walks and dispatches exactly like any
