...
func TestSchedulerMetadataFields(t *testing.T) {
	...
	if len(snap.Capabilities) != 4 || snap.Capabilities[0] != "status" || snap.Capabilities[1] != "models.pull" || snap.Capabilities[2] != "models.list" || snap.Capabilities[3] != "models.delete" {
		t.Errorf("Capabilities = %v, want [status models.pull models.list models.delete]", snap.Capabilities)
	}
