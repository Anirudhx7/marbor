// Package service (see service.go's package doc) also owns Node Agent TLS
...
// nodeagent (as .local/core/P24-TLS-DESIGN.md section 9 originally
// suggested): each platform's Install (service_linux.go/service_darwin.go/
// service_windows.go) must call this at install time, and package nodeagent
// already imports package service, so package service importing back from
// nodeagent would be a compile-time cycle. This file never imports
// nodeagent - only stdlib.
package service
