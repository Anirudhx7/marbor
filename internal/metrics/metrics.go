...
var (
	...
	prefixLocalityHitsTotal = promauto.NewCounter(prometheus.CounterOpts{
		...
		Help: "Requests whose prefix hash matched a recorded prefix-locality routing hint",
	})

	prefixLocalityMissesTotal = promauto.NewCounter(prometheus.CounterOpts{
		Name: "marbor_prefix_locality_misses_total",
		Help: "Requests with no matching prefix-locality routing hint",
	})
)
...
// routing hint. Does not imply that node was actually selected -
