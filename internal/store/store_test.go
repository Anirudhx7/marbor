...
func TestNodeOverrides_VRAMOverrides(t *testing.T) {
	s := openTestDB(t)
	...
	if ov.VRAMOverrides == nil || len(*ov.VRAMOverrides) != 2 || (*ov.VRAMOverrides)["llama3.1:8b"] != 8192 || (*ov.VRAMOverrides)["mixtral"] != 26000 {
		t.Fatalf("VRAMOverrides = %v, want {llama3.1:8b:8192 mixtral:26000}", ov.VRAMOverrides)
	}
	...
	ov = ovs["node1"]
	if ov.VRAMOverrides == nil || len(*ov.VRAMOverrides) != 2 {
		t.Fatalf("VRAMOverrides after unrelated merge = %v, want unchanged", ov.VRAMOverrides)
	}
	...
	ov = ovs["node1"]
	if ov.VRAMOverrides == nil || len(*ov.VRAMOverrides) != 0 {
		t.Errorf("VRAMOverrides after clear = %v, want empty", ov.VRAMOverrides)
	}
}
