...
// admin_sessions/user_sessions tables - the token is already a
...
func (s *sqliteStore) migrate() error {
	...
	stmts := []string{
		...
		// operator-declared physical GPU index list for this node,
		// JSON-encoded (e.g. "[0,1]") - same JSON-in-TEXT convention as
		...
		// (per-action token scoping) is NOT a secret - it's a plaintext
		...
		// Default 'admin' matches every row from before this column existed
		// (unprefixed, full-scope-by-fallback) token.
		// scheme (added by a later fix once TLS support required distinguishing
		// the agent's own transport scheme) is the Marbor Agent's OWN transport scheme
		...
		// node_control holds the per-node ControlDriver configuration - none of these columns
		...
		// marbor's own /v1/chat/completions). No secrets here - the at-rest encryption guard doesn't apply.
	...
	for _, col := range []string{
		...
		// TOFU-pinned SHA-256 fingerprint of the Marbor agent's TLS cert.
		...
		// Per-action Marbor Agent token scoping - see marbor_agent's
		...
		// Per-request routing explainability. routing_reason is the short
		...
		// Follow-up fix from code review: the Requests page's list view reads
		...
		// Deployment-aware placement - structured parallelism metadata
		...
		// fabricated 0 - the same absence-not-fabrication convention already applied to TPOT here
		...
		// Operator-declared per-model VRAM size overrides (MB), JSON
		...
		// went unnoticed here while schema_version was still stamped
...
func (s *sqliteStore) migrateRenameNodeAgentTable() error {
	...
	// Idempotent against a node_agent frozen at a shape older than the scope/
...
func (s *sqliteStore) DeleteNode(name string) error {
	...
	// transaction - previously these ran as separate autocommit
	...
	// leave the real row (and its secret token) orphaned. Only delete
	...
	if agentHost != "" {
		...
		if !stillShared {
			...
			// dangling-secret concern, so it is cleaned up here even
...
// INSERT OR REPLACE: REPLACE deletes and reinserts the
...
func (s *sqliteStore) NodeOverrides() (map[string]NodeOverride, error) {
	...
	for rows.Next() {
		...
		if indicesJSON != "" {
			...
			// whole row (the same discipline as elsewhere: a bad reading becomes "unknown",
...
// --- Node ControlDriver config (per-node) ---
...
func (s *sqliteStore) AllSettings() (map[string]string, error) {
	...
	for rows.Next() {
		...
		if sensitiveSettingKeys[k] {
			...
			// ciphertext string instead of the real value. The same discipline as elsewhere: a
...
func (s *sqliteStore) InsertBenchmarkRun(run BenchmarkRun) error {
	_, err := s.db.Exec(
		`INSERT INTO benchmark_runs
			(node, model, n, cold_p50_ms, cold_min_ms, cold_max_ms, cold_p95_ms, cold_p99_ms,
			 warm_p50_ms, warm_min_ms, warm_max_ms, warm_p95_ms, warm_p99_ms,
...
func (s *sqliteStore) ListBenchmarkRuns(limit int) ([]BenchmarkRun, error) {
	...
	rows, err := s.db.Query(
		`SELECT id, node, model, n, cold_p50_ms, cold_min_ms, cold_max_ms, cold_p95_ms, cold_p99_ms,
			warm_p50_ms, warm_min_ms, warm_max_ms, warm_p95_ms, warm_p99_ms,
			cold_tpot_p50_ms, warm_tpot_p50_ms, speedup_x, created_at
...
// nullableFloat64 converts a *float64 to the sql.NullFloat64 a nullable REAL
// column expects - nil means "not computable" (absence, never 0).
...
// nil when the column was NULL (absence, never a fabricated 0).
