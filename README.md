...
| | Per-node in-flight cap | Optional hard `max_in_flight_per_node` (global) with a per-node override. A node at/over its cap is shed immediately - failover/cloud/503 - instead of queued, for operators who need instant overflow rather than piling onto a slow node. |
