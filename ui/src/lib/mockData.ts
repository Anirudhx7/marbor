...
export const mockGPUNodes: GPUNode[] = [
  ...
  // Multi-host replica demo pair: gpu-node-07 and
  // gpu-node-08 declare each other as a valid 2-host TP replica, showing
  // the "Worker (non-schedulable)" badge + "Part of replica" head-link on
  // the worker, and the resolved schedulingRole on the head (no badge -
  // its card is what workers link to).
  {
    id: 'node-7',
    name: 'gpu-node-07',
    host: '10.0.0.17',
    ...
    parallelismType: 'tp',
    parallelismWidth: 8,
    effectiveRequiredGPUs: 8,
    vramTotalMB: 80 * GB,
    vramUsedMB: Math.round(61.0 * GB),
    vramSource: 'nvidia',
    powerDrawW: 340,
    cpuPercent: 38,
    temperature: 70,
    ...
    uptime: '3d 2h',
    loadedModels: [
      { name: 'meta-llama/Llama-3.1-405B-Instruct', sizeVram: Math.round(61.0 * GiB) },
    ],
    healthHistory: Array(60).fill(0).map(() => 97 + Math.random() * 3),
    coldStarts: 0,
    tokensTotal: 412000,
    avgLatencyMs: 210,
    warmHitRatio: 1.0,
  ...
  },
  {
    ...
    pendingPrewarmMB: 0,
    uptime: '3d 2h',
    // A worker never serves a routable endpoint of its own - no loadedModels
    // list is meaningful here, matching real TP/PP deployments where only
    // the head process binds the OpenAI-compatible port.
    loadedModels: [],
    healthHistory: Array(60).fill(0).map(() => 97 + Math.random() * 3),
    coldStarts: 0,
    tokensTotal: 0,
  ...
  // Unresolved-replica demo pair: gpu-node-09 declares gpu-node-10 as a
  // member, but gpu-node-10 has a conflicting declaration (different head) -
  // the fleet-wide closure can't agree, so both resolve "unresolved" until
  // an operator reconciles the two declarations. Shows the destructive-red
  // "Unresolved replica" badge, distinct from the worker's informational one.
  {
    id: 'node-9',
    name: 'gpu-node-09',
    host: '10.0.0.19',
    ...
    vramTotalMB: 192 * GB,
    vramUsedMB: Math.round(140.0 * GB),
    vramSource: 'agent',
    powerDrawW: 380,
    cpuPercent: 44,
    temperature: 74,
  ...
  },
  {
    ...
    // resolve RoleUnresolved.
    replicaPeers: { members: ['gpu-node-09', 'gpu-node-10'], head: 'gpu-node-10' },
    schedulingRole: 'unresolved',
    vramTotalMB: 192 * GB,
    vramUsedMB: Math.round(2.1 * GB),
    vramSource: 'agent',
    powerDrawW: 90,
    cpuPercent: 6,
    temperature: 52,
...
const mockRoutingDecisions: Record<string, RoutingDecision> = {
  'req-a1b2c3d4e5f6': {
    ...
    score: 78.5,
    components: [
      { name: 'warm_model_resident', raw: 1, weight: 50, value: 50, phase: 'locality' },
      { name: 'free_vram_headroom', raw: 0.4, weight: 20, value: 8, phase: 'locality' },
      { name: 'inverse_queue_depth', raw: 1, weight: 15, value: 15, phase: 'predicted_performance' },
      { name: 'node_health', raw: 1, weight: 10, value: 10, phase: 'reliability' },
      { name: 'success_rate', raw: 1, weight: 5, value: 5, phase: 'reliability' },
      { name: 'cooldown_penalty', raw: 0, weight: -50, value: 0, phase: 'reliability' },
      { name: 'stale_telemetry_penalty', raw: 0, weight: -50, value: -9.5, phase: 'reliability' },
    ],
    excluded: [
      { node: 'gpu-node-04', reason: 'unhealthy' },
      { node: 'gpu-node-05', reason: 'over_capacity' },
    ],
  },
  'req-b2c3d4e5f6a1': {
    node: 'gpu-node-02',
  ...
  'req-f6a1b2c3d4e5': {
    ...
    components: [
      { name: 'warm_model_resident', raw: 1, weight: 50, value: 50, phase: 'locality' },
      { name: 'free_vram_headroom', raw: 0.3, weight: 20, value: 6, phase: 'locality' },
      { name: 'inverse_queue_depth', raw: 0.2, weight: 15, value: 3, phase: 'predicted_performance' },
      { name: 'node_health', raw: 0.2, weight: 10, value: 2, phase: 'reliability' },
      { name: 'success_rate', raw: 0.2, weight: 5, value: 1, phase: 'reliability' },
      { name: 'cooldown_penalty', raw: 0, weight: -50, value: 0, phase: 'reliability' },
      { name: 'stale_telemetry_penalty', raw: 0, weight: -50, value: 0, phase: 'reliability' },
    ],
    excluded: [
